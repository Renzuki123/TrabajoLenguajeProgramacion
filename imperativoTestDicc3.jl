...
while(true)
    ...
    tiempoInput = [] # Acá guardaremos los tiempos por cada readline del jugador
    ...
    function jugar()
        while (!is_game_over)
            ...
            tiempoInp = @elapsed inp = input() # guardará el resultado en inp y el tiempo en tiempoInp
            chomp(inp)
            inpList = split(inp) # Convierte a lista y separa cada elemento del input
            
            push!(tiempoInput, tiempoInp)
            if (inp == "exit")
                displayFinalField()
                exit()
    ...
    Dicc[jugador] = tiempo
    suma = sum(tiempoInput)
    tiempoAlgoritmo = tiempo - suma
    println("Tiempo de write: ", suma)
    println("Tiempo de algoritmo: ", tiempoAlgoritmo)
